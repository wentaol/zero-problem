...
static const char* const js_load_problems_19[] = {
	...
	R"*(		(c+1)+"  "+d.metadata+"  "+d.wr[0]+"%/"+d.wr[1]+"%"));a=n.length;for(b=0;b<a;++b){c=n[b];for(var f,h=c.st,g=[],u=h.length,q=0;q<u;++q)f=h.charCodeAt(q)-64,d=f%3,g.push(d),f=(f-d)/3,d=f%3,g.push(d),f=(f-d)/3,d=f%3,g.push(d);d=g;f=[];for(g=h=0;21>g;++g)for(u=0;21>u;++u)0===g||20===g||0===u||20===u?f.push(4):(f.push(d[h]),++h);c.st=f;c.lm=D(c.lm,0);c.cl="B"===c.cl?1:2;c.va[0]=E(c.va[0]);c.va[1]=E(c.va[1])}a=2*n.length;for(b=0;b<a;++b)p.push(new k(b,0));F();a=3*n.length;for(b=0;b<a;++b)(new e(b)).N();a=2*n.length;for(b=)*",
